"""
Prepare the DermNet dataset into a JSONL manifest compatible with MultiMeditron (Skin Expert).

- Downloads DermNet via KaggleHub
- Extracts nested archives if present
- Infers class labels from directory structure
- Optionally de-duplicates against training datasets using MD5 hashes
...
@dataclass
class DermNetPrepArguments:
    kaggle_dataset: str = field(
        default="shubhamgoel27/dermnet",
        metadata={"help": "Kaggle dataset slug for DermNet"},
    )
...
def main():
    parser = HfArgumentParser(DermNetPrepArguments)
    ...
    logger.info("Preparing DermNet dataset (Skin Expert)")
    ...
    with args.output_jsonl.open("w", encoding="utf-8") as fout:
        for src in tqdm(src_images, desc="Preparing DermNet"):
